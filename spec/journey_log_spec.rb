...
describe JourneyLog do
  ...
  # let(:inc_journey_class1) { double :inc_journey_class, new: inc_journey1 } (no longer need multiple journey_class doubles, use allow statements instead)
  ...
  describe '#start' do
    it 'starting a journey without ending curreny will save incomplete journey to history' do
      allow(journey_class).to receive(:new).and_return(inc_journey1)
  ...
  describe '#finish' do
    it 'touching out witout touching in will return an imcomplete journey to the array' do
      allow(journey_class).to receive(:new).and_return(inc_journey2)
